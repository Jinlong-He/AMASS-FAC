...
We manually check on a virtual machine (4-core CPU, 2G RAM with Android 13.0) whether these 100 apps are indeed task/fragment-container unbounded by verifying whether the witness cycles reported by {\tool} are executable. 
...
Finally, considering that the crashes and reboots of mobile applications can be caused by a variety of reasons and it is common for mobile apps to experience such issues, to demonstrate causality between the crashes and task/fragment-container unboundedness, we use the Monkey tool\footnote{https://developer.android.com/studio/test/other-testing-tools/monkey} to stress-test the 74 apps in Table~\ref{tab:threat-fd-task}---\ref{tab:threatpop}. We set the timeout period of Monkey to be 30 minutes. 
%	
...
%
Namely, 14 out of 74 apps (around 19\%) apps end up with abnormal behaviors which shows that, compared to the random testing, the repeated executions of witness cycles indeed dramatically increase the chances of exposing abnormal behaviors. 

\revision{Furthermore, we use ``adb logcat'' command to record the log of the app when executing the witness cycles repeatedly, it shows that 
...
\end{itemize}
We use ``adb shell dumpsys meminfo'' to monitor the memory usage when executing the witness cycles repeatedly and record the memory usage increment for each cycle, the results are shown in Table~\ref{tab:bound-mem}. It shows that the average memory increment per cycle for app restart or reboot is comparable, while the average memory increment per cycle for app crashe is much smaller than the other two, moreover, the average memory increment per cycle for fragment container unboundedness is smaller than task unboundedness.
Since the memory increment per cycle for apps crash is small, when executing the witness cycle repeatedly, the number of existing layers will rech the maximum system limit before causing the memory management strategies. This also explains why fragment unboundedness requires more iterations of the witness cycles to execute, as the memory usage increment per cycle is lower compared to task unboundedness.
...
\begin{table}[htbp]   
	\begin{center}   
		\begin{tabular}{|c|c|c|}   
			\hline   \textbf{unbound. vulnerability} &\textbf{Abnormal behavior} & \textbf{Avg. size per cycle}   \\   
			\hline
			...
			\hline
		\end{tabular}   
		\caption{\revision{Memory usage per cycle between different abnormal behaviors}\label{tab:bound-mem}}
	\end{center}   
\end{table}
...
To compare the impact of different extractors on TaskDroid, for each app, we run {\sf AMASSAnalyzer} on the three models constructed by the three extractors for the app. We summarize the results in Figure~\ref{fig:cmp-extractor-ubd}, where we can see that the number of task unbounded apps is the greatest when ICCBot$_{\AMASS}$ is used. This is because ICCBot$_{\AMASS}$ extracts more models than the other two extractors since it utilizes the dynamic approach to extract the models from the commercial apps that cannot be decompiled. In Table~\ref{tab:threatpop}, by repeatedly executing the witness cycles, we confirm that among these apps whose models are constructed dynamically by ICCBot$_{\AMASS}$, 9 apps are indeed task unbounded. 
%	
