...
\revision{Furthermore, we use ``adb logcat'' command to record the log of the app when executing the witness cycles repeatedly, it shows that all the apps which ending up with app restart and rebooting of the device throw the ``android.os.DeadObjectException'' exeption, and all the apps which ending up with app crash report the ``createSurface error Out of memory'' error. The ``android.os.DeadObjectException'' exeption is mainly caused by killing the process due to the lacks of the resources such that memory, CPU, e.g., we use ``adb shell top'' command to monitor the usage of memory of the apps, it shows that the usage of memory is increasing when executing the witness cycles repeatedly.
