...
\revision{To ensure the reliability of the auditing source code, we ask 4 individuals to audit the source code of Android OS individually and construct the control flows of the two functions.}
...
\revision{We select 20 apps from F-Droid to validate the formal semantics, with 10 apps selected to cover as many launch modes and intent flags as possible for validating the semantics of $\AOAMASS$, and 10 apps selected to cover as many types of fragment transactions as possible for validating the semantics of $\FOAMASS$.}
%%%%%%%%%% original version. 
...
%%%%%%%%%% original version. 
Nevertheless, these apps do not cover the vast number of cases in the definition of the semantics of $\AOAMASS$ and $\FOAMASS$, since there are exponentially many different combinations of intent flags. 
...
Therefore, in addition to these apps, we design a special Android app ValApp\footnote{available at https://github.com/Jinlong-He/ValApp/tree/main}, aiming at covering all the cases in the definition of the semantics of $\AOAMASS$ and $\FOAMASS$ models. 
...
\end{enumerate}
\revision{Note that in the second step, generating the sequence of click events from a transition rule requires manual effort. For a transition rule of the form $\tau = A\xrightarrow{\alpha(\phi)} B$, we attemp to click all the widgets in the activity $A$ to find a sequence of click events to launch the activity $B$.  However, There maybe multiple sequences of click events to launch $B$, 
