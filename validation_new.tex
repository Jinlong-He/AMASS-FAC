...
The goal of this section is to validate the formal semantics of {\AMASS} models defined in Section~\ref{sec:amass}.  
...
The souce code are shown in Appendix~\ref{sec:souce-code}.
...
\subsubsection{Auditing the source code for $\AOAMASS$}
...
Let us have a closer look at the source code of the procedures called directly or indirectly by startActivityInner(), as illustrated in Figure~\ref{code-startActivityInner}--\ref{code-reuseOrCreateTask}. 
\begin{itemize}
...
\subsection{Validation of the formal semantics by testing Android apps}\label{sec:val-exp}

To validate the semantics of $\AOAMASS$, we select 10 apps from F-Droid. These apps are selected to cover as many launch modes and intent flags as possible.  Similarly, to validate the semantics of $\FOAMASS$, we select 10 apps from F-Droid to cover as many fragment transactions as possible. Nevertheless, these apps are far from covering the vast number of cases in the definition of the semantics of $\AOAMASS$ and $\FOAMASS$, \jinlong{the main reason is that the intent flags may interfere with each other. For example, $\ctpflag$ will be ignored when $\ctkflag$ is set, hence real world apps may not use $\ctkflag$ and $\ctpflag$ together. However, we need to validate all possible combinations of intent flags to ensure the correctness of the semantics of $\AOAMASS$, even though most combinations are not used in the real world apps.}
