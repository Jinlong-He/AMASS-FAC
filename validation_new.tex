...
\revision{The details of the auditing are relegated to Appendix~\ref{app:code-audit-aomass}. }
...
\end{itemize}
\revision{The details of the auditing are relegated to Appendix~\ref{app:code-audit-fomass}.}
...
To validate the semantics of $\AOAMASS$ empirically, we select 10 apps from F-Droid. These apps are selected to cover as many launch modes and intent flags as possible.  Similarly, to validate the semantics of $\FOAMASS$, we select 10 apps from F-Droid to cover as many types of fragment transactions as possible. \revision{Nevertheless, these apps do not cover the vast number of cases in the definition of the semantics of $\AOAMASS$ and $\FOAMASS$, since there are exponentially many different combinations of intent flags. 
...
Therefore, in addition to these apps, we design a special Android app \revision{ValApp\footnote{available at https://github.com/Jinlong-He/ValApp/tree/main}}, aiming at covering all the cases in the definition of the semantics of $\AOAMASS$ and $\FOAMASS$ models. }

\revision{The reason that we only select 10 apps (from F-Droid plus ValApp) to validate the semantics of $\AOAMASS$ or $\FOAMASS$, instead of %validate the semantics by 
%	
...
it is hard to apply the automated method to a large number of existing apps,  since generating sequences of click events for transitions is still a manual process. As a result, we mainly rely on ValApp to carry out semantics validation. 
%
