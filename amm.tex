...
\jinlong{
Moreover, after an activity call startActivity() or startActivityForResult() to start another activity, the caller activity could call finish() function, this function allows the caller activity to be finished immediately. 
...
The real activity indeed affects the evolution of the task stack. For instance, if the real activity of some task is $B$ and the task is not the main task, then attempting to push an instance of $B$ to the task will not modify the task. 
}
\hide{
\begin{itemize}
...
\jinlong{The add (resp. remove) action will push (resp. pop) the fragment of a fragment container, but replace action will pop all fragments in the fragment container and push the fragment.}
...
We use two F-Droid apps com.quaap.launchtime (LaunchTime Homescreen)\footnote{available at https://github.com/quaap/LaunchTime/blob/master/} and org.janb.shoppinglist (ShoppingList)\footnote{available at https://github.com/GroundApps/ShoppingList/blob/master/} to illustrate the motivation of this work. 
\subsubsection{LaunchTime Homescreen}
The app LaunchTime Homescreen has two activities, MainActivity and SettingsActivity. From the source code of LaunchTime Homescreen in Figure~\ref{code-launchtime}, we can see that the launch mode of MainActivity is singleInstance and the launch mode of SettingsActivity is standard (the default mode). Moreover, MainActivity can start SettingsActivity by calling startActivity(settingsIntent) function in line 3942 of the MainActivity.java file, where the settingsIntent contains the intent flags FLAG\_ACTIVITY\_NEW\_TASK.
SettingsActivity can start MainActivity by calling startActivity(main) function without any intent flags in line 142 of the SettingsActivity.java file , but after starting MainActivity, finish() will be called in line 144.
